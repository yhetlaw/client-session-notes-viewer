...
export function ClientDetailsPage() {
  ...
  const handleSubmitNote = (e: React.FormEvent) => {
    e.preventDefault();
    if (newNote.trim()) addSessionNote({ clientId: client?.id ?? 0, note: newNote }, { onSuccess: () => setNewNote('') });
  };
      ...
      <Paper sx={{ p: 3, maxWidth: 800 }}>
        ...
        <Box mt={4}>
          ...
          <Box component='form' onSubmit={handleSubmitNote} mt={3}>
            <TextField fullWidth multiline rows={3} label='New Session Note' value={newNote} onChange={(e) => setNewNote(e.target.value)} margin='normal' />
